...
class xbee_obs(object):
    ...
    def __init__(self):
        ...
        self.s = rospy.Service('xbee_observation', XBeeArrayService, self.handle_xbee_obs)
        ...
        if self.xbeeHandler.init_port(self.port, self.baud)>=0:
            ...
            rospy.logerr("Could NOT initialize the serial port, xbee scanning is exiting")
    ...
    def handle_xbee_obs(self, data):
        ...
        for i in range(len(self.addr)):
            obsList.append(self.xbeeHandler.send_remote_at(frame_id="0", dest_addr_long = self.addr[i].decode("hex")))
            print obsList[i]
